    ...
    StoreT: Clone + Send + Store,
{
    ...
    pub fn new_from_bytes(
        ...
        let component = if precompiled {
            ...
            unsafe { Component::deserialize(&library.environment.engine, bytes).map_err(PluginError::LoadWasm)? }
        } else {
            Component::from_binary(&library.environment.engine, bytes).map_err(PluginError::LoadWasm)?
        };
        ...
        PathT: AsRef<path::Path>,
    {
        let component = if precompiled {
            // SAFETY: we are assuming the precompiled .cwasm is safe!
            unsafe { Component::deserialize_file(&library.environment.engine, path).map_err(PluginError::LoadWasm)? }
        } else {
            Component::from_file(&library.environment.engine, path).map_err(PluginError::LoadWasm)?
        };
