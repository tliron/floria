    ...
    pub fn to_guest_any<AnnotatedT>(
        ...
        match value {
            ...
            normal::Variant::List(list) => {
                ...
                let resource =
                    nested_list_access.call_constructor(&mut self.host, &vector).map_err(PluginError::CallWasm)?;
        ...
        AnnotatedT: Default,
    {
        match value {
            ...
            dispatch::Any::NestedList(resource) => {
                let nested_list_access = self.bindings.floria_plugins_dispatch().nested_list();
                let vector = nested_list_access.call_get(&mut self.host, resource).map_err(PluginError::CallWasm)?;
            ...
            dispatch::Any::NestedMap(resource) => {
                let nested_map_access = self.bindings.floria_plugins_dispatch().nested_map();
                let key_value_pairs =
                    nested_map_access.call_get(&mut self.host, resource).map_err(PluginError::CallWasm)?;
                resource.resource_drop(&mut self.host).map_err(PluginError::CallWasm)?;
