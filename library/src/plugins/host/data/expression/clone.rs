    ...
    StoreT: Store,
{
    ...
    pub fn deep_clone_expression(
        &mut self,
        expression: bindings::Expression,
    ) -> Result<bindings::Expression, PluginError> {
        Ok(match expression {
            ...
            bindings::Expression::Map(resource) => {
                ...
                let map = bindings::HostMapResource::new(self, key_value_pairs).map_err(PluginError::CallWasm)?;
                map.into()
            }
            ...
            bindings::Expression::Custom(resource) => {
                ...
                let custom = bindings::HostCustomResource::new(self, kind, inner).map_err(PluginError::CallWasm)?;
                custom.into()
            }
...
pub trait ShallowClone {
    ...
    ///
    /// Thus uou *must* call [deep_clone_expression](PluginHost::deep_clone_expression) on a
    /// shallow clone.
