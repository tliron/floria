    ...
    StoreT: Store,
{
    ...
    pub fn expression_to_bindings(&mut self, expression: Expression) -> Result<bindings::Expression, Problem> {
        match expression {
            ...
            Expression::Map(map) => {
                ...
                let map = bindings::HostMapResource::new(self, map).into_wasm_resource_problem("new map")?;
                Ok(map.into())
            }

            Expression::Custom(kind, inner) => {
                let inner = self.expression_to_bindings(*inner)?;
                let custom = bindings::HostCustomResource::new(self, kind.into(), inner)
                    .into_wasm_resource_problem("new custom")?;
                Ok(custom.into())
            }
    ...
    pub fn expression_from_bindings(&mut self, expression: bindings::Expression) -> Result<Expression, Problem> {
        match expression {
            ...
            bindings::Expression::List(resource) => {
                let list = self.resources.delete(resource).into_wasm_resource_problem("delete list")?.inner;
                let list = self.expressions_from_bindings(list)?;
                Ok(list.into())
            }

            bindings::Expression::Map(resource) => {
                let map = self.resources.delete(resource).into_wasm_resource_problem("delete map")?.inner;
                let map = self.expression_map_from_bindings(map)?;
                Ok(map.into())
            }

            bindings::Expression::Custom(resource) => {
                let custom = self.resources.delete(resource).into_wasm_resource_problem("delete custom")?;
            ...
            bindings::Expression::Call(resource) => {
                let call = self.resources.delete(resource).into_wasm_resource_problem("delete call")?;
    ...
    ) -> Result<BTreeMap<Expression, Expression>, Problem> {
