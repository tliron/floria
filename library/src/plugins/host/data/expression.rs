    ...
    StoreT: Store,
{
    ...
    pub fn expression_to_bindings(&mut self, expression: Expression) -> Result<bindings::Expression, PluginError> {
        match expression {
            ...
            Expression::Map(map) => {
                let mut key_value_pairs = Vec::default();
            ...
            Expression::Call(call) => {
                ...
                let resource =
                    self.resources.push(BindingsCall::new(call.plugin.into(), call.function.into(), arguments))?;
    ...
    pub fn expression_from_bindings(&mut self, expression: bindings::Expression) -> Result<Expression, PluginError> {
        match expression {
            ...
            bindings::Expression::Call(resource) => {
                ...
                Ok(Call::new(plugin, function, expression_arguments, false).into())
            }
        }
    }
}
