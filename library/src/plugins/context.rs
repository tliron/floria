    ...
    StoreT: Store,
{
    ...
    pub fn dispatch_plugin_ref(&mut self, plugin_id: &ID) -> Result<DispatchPluginRef<StoreT>, Problem> {
        self.dispatch_plugins
            .pin()
            .get(plugin_id)
            .cloned()
            .ok_or_else(|| MissingError::as_problem(plugin_id).via(PluginError))
    }
    ...
    pub fn add_dispatch_plugin(
        ...
        bytes: &[u8],
        precompiled: bool,
    ) -> Result<DispatchPluginRef<StoreT>, Problem>
    where
        StoreT: Clone + Send,
    {
    ...
    pub fn maybe_load_dispatch_plugin_ref(&mut self, plugin_id: &ID) -> Result<DispatchPluginRef<StoreT>, Problem>
    where
        StoreT: Clone + Send,
    {
        let (url, precompiled) = match self.dispatch_plugins.pin().get(plugin_id) {
            ...
            None => match self.store.get_plugin(plugin_id)? {
                ...
                None => return Err(MissingError::as_problem(plugin_id).via(PluginError)),
            },
        };

        let url = self.url_context.url(&url).via(PluginError)?;

        match url.local() {
            Some(path) => self.load_dispatch_plugin(plugin_id.clone(), path, precompiled),

            None => {
                let mut reader = url.open().via(PluginError)?;
                let mut bytes = Vec::default();
                reader.read_to_end(&mut bytes).via(PluginError)?;
