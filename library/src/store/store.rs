    ...
    StoreT: Store,
{
    fn get_entity_as_expression(&self, id: &ID) -> Result<Option<Expression>, StoreError> {
        let variant = match id.kind {
            Kind::Class => self.get_class(&id)?.map(|class| Ok(class.into())),
            Kind::VertexTemplate => {
            ...
            Kind::Vertex => self.get_vertex(&id)?.map(|vertex| vertex.into_expression(false, self)),
            Kind::Edge => self.get_edge(&id)?.map(|edge| edge.into_expression(false, self)),
        };
