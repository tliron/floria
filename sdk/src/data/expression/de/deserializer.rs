...
pub struct ExpressionDeserializer<'own> {
    expression: &'own Expression,
}

impl<'own> ExpressionDeserializer<'own> {
    /// Constructor
    pub fn new(expression: &'own Expression) -> Self {
        Self { expression }
    }

    fn incompatible_type_error(&self) -> DeserializeError {
        DeserializeError(format!("incompatible type: |error|{}|", self.expression.type_name()))
    }

    fn incompatible_value_error(&self) -> DeserializeError {
        DeserializeError(format!("incompatible value: |error|{}|", self.expression.type_name()))
    }
}

// See: https://serde.rs/impl-deserializer.html

impl<'de, 'own> de::Deserializer<'de> for &'own mut ExpressionDeserializer<'de> {
