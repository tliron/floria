...
impl Entity {
    /// Constructor.
    pub fn new(id: Id, expression: Expression) -> Self {
        Self { id, expression }
    }

    /// Constructor.
    pub fn new_from(expression: Expression) -> Option<Self> {
        Id::new_from(&expression).map(|id| Self::new(id, expression))
    }
    ...
    pub fn is_in_class(&self, class_id: &Id) -> Result<bool, String> {
        match self.expression.into_get("class_ids") {
            Some(class_ids) => match class_ids {
                Expression::List(class_ids) => {
                    for class_id_ in &class_ids.list().inner {
                        match class_id_ {
                            Expression::Text(class_id_) => {
    ...
    pub fn get_containing_vertex_id(&self) -> Result<Option<Id>, String> {
        match &self.id.kind {
            Kind::Vertex => match self.expression.into_get("containing_vertex_id") {
                Some(containing_vertex_id) => match containing_vertex_id {
                    Expression::Text(containing_vertex_id) => Ok(Some(Id::parse(Kind::Vertex, containing_vertex_id))),
    ...
    pub fn get_origin_template_id(&self) -> Result<Option<Id>, String> {
        match &self.id.kind {
            Kind::Vertex => match self.expression.into_get("origin_template_id") {
                Some(origin_template_id) => match origin_template_id {
                    Expression::Text(origin_template_id) => {
                        Ok(Some(Id::parse(Kind::VertexTemplate, origin_template_id)))
                    }
            ...
            Kind::Edge => match self.expression.into_get("origin_template_id") {
                Some(origin_template_id) => match origin_template_id {
                    Expression::Text(origin_template_id) => Ok(Some(Id::parse(Kind::EdgeTemplate, origin_template_id))),
    ...
    pub fn get_contained_vertex_ids(&self) -> Result<Vec<Id>, String> {
        match &self.id.kind {
            Kind::Vertex => match self.expression.into_get("contained_vertex_ids") {
                Some(contained_vertex_ids) => match contained_vertex_ids {
                    Expression::List(contained_vertex_ids) => {
                        let contained_vertex_ids = contained_vertex_ids.list();
                        ...
                        for contained_vertex_id in &contained_vertex_ids.inner {
                            match contained_vertex_id {
                                Expression::Text(contained_vertex_id) => {
    ...
    pub fn get_outgoing_edge_ids(&self) -> Result<Vec<Id>, String> {
        match &self.id.kind {
            Kind::Vertex => match self.expression.into_get("outgoing_edge_ids") {
                Some(outgoing_edge_ids) => match outgoing_edge_ids {
                    Expression::List(outgoing_edge_id) => {
                        let outgoing_edge_ids = outgoing_edge_id.list();
                        ...
                        for outgoing_edge_id in &outgoing_edge_ids.inner {
                            match outgoing_edge_id {
                                Expression::Text(outgoing_edge_id) => {
    ...
    pub fn get_incoming_edge_ids(&self) -> Result<Vec<Id>, String> {
        match &self.id.kind {
            Kind::Vertex => match self.expression.into_get("incoming_edge_ids") {
                Some(incoming_edge_ids) => match incoming_edge_ids {
                    Expression::List(incoming_edge_ids) => {
                        let incoming_edge_ids = incoming_edge_ids.list();
                        ...
                        for incoming_edge_id in &incoming_edge_ids.inner {
                            match incoming_edge_id {
                                Expression::Text(incoming_edge_id) => {
    ...
    pub fn get_source_vertex_id(&self) -> Result<Id, String> {
        match &self.id.kind {
            Kind::Edge => match self.expression.into_get("source_vertex_id") {
                Some(source_vertex_id) => match source_vertex_id {
                    Expression::Text(source_vertex_id) => Ok(Id::parse(Kind::Vertex, source_vertex_id)),
    ...
    pub fn get_target_vertex_id(&self) -> Result<Id, String> {
        match &self.id.kind {
            Kind::Edge => match self.expression.into_get("target_vertex_id") {
                Some(target_vertex_id) => match target_vertex_id {
                    Expression::Text(target_vertex_id) => Ok(Id::parse(Kind::Vertex, target_vertex_id)),
    ...
    pub fn get_property(&self, property_name: &str) -> Result<Option<Property<'_>>, String> {
        let object = self
            .expression
            .into_get("properties")
            .ok_or_else(|| format!("entity {} is missing properties", self.id))?;
