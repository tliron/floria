...
impl PartialEq for Any {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::NestedList(list), Self::NestedList(other_list)) => list.to_list() == other_list.to_list(),
            (Self::NestedMap(map), Self::NestedMap(other_map)) => map.to_map() == other_map.to_map(),
            _ => false,
        }
    }
}
...
impl PartialOrd for Any {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match (self, other) {
            ...
            (Self::NestedList(list), Self::NestedList(other_list)) => list.to_list().partial_cmp(other_list.to_list()),
            (Self::NestedMap(map), Self::NestedMap(other_map)) => map.to_map().partial_cmp(other_map.to_map()),
            _ => None,
        }
    }
}
...
impl Ord for Any {
    fn cmp(&self, other: &Self) -> Ordering {
        match (self, other) {
            ...
            (Self::NestedList(list), Self::NestedList(other_list)) => list.to_list().cmp(other_list.to_list()),
            (Self::NestedMap(map), Self::NestedMap(other_map)) => map.to_map().cmp(other_map.to_map()),
            ...
            (Self::NestedList(_), _) => Ordering::Less,

            (Self::NestedMap(_), _) => Ordering::Less,
        }
    }
}
        ...
        HasherT: Hasher,
    {
        match self {
            ...
            Self::NestedList(list) => {
                state.write_u8(8);
                list.to_list().hash(state);
            }

            Self::NestedMap(map) => {
                state.write_u8(9);
                map.to_map().hash(state);
            }
        }
    }
}
