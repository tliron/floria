...
/// Not of types.
pub fn not_of_types(expression: &Expression, types: &[&str]) -> DispatchError {
    let types: Vec<_> = types.into_iter().map(|t| format!("|name|{}|", t)).collect();
    format!("not {}: |error|{}|", types.join(", "), expression.type_name())
}

/// Not of types.
pub fn not_of_types_for(name: &str, expression: &Expression, types: &[&str]) -> DispatchError {
    format!("{} {}", name, not_of_types(expression, types))
}

/// Not the same type.
pub fn not_same_type(left: &Expression, right: &Expression, operator: &str) -> DispatchError {
